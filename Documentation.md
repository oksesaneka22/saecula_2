...
│   └── world3d/             # 3D Світ (World3D.gd, World3D.tscn, WorldResourceNode3D.gd, BuildingGhost3D.gd)
```
...
- **`BuildMenuUI` (`Control`):** Меню каталогу будівель та креслень. Відкривається/закривається на `B` або `Escape`. Надає вибір доступних монументальних споруд та активує режим розміщення.
...
- **Керування:**
  ...
  - `ЛКМ`: 3D Raycast проекція променя курсора у світ (`project_ray_origin` / `project_ray_normal`). Клік по ресурсу видобуває його; клік у режимі будівництва підтверджує фіксацію креслення на землі.
...
- **Вузол `World3D`:**
  - Масштаб карти: 80x80 тайлів (160м x 160м), що створює достатньо простору для великих баз і комплексів будівель.
  ...
  - Спавнить гравця `Player3D`, камеру `RTSCamera3D` та креслення-привид `BuildingGhost3D`.
...
## 9. Система розміщення великих споруд та креслень (Building Placement System)

### 9.1. Вимоги до габаритів будівель (>= 8x8 тайлів)
Усі капітальні споруди спроєктовані як масштабні об'єкти розміром не менше **8x8 тайлів** (16м x 16м у 3D просторі):
1. **Табірне вогнище (`campfire.tres`):**
   - Розмір: **8x8 тайлів** (16x16м, 64 клітинки).
   - Призначення: центральне ритуальне вогнище поселення, зона обігріву та збору колоністів.
   - Вартість: 4 дерева + 4 каменю.
2. **Головний склад ресурсів (`stockpile.tres`):**
   - Розмір: **10x10 тайлів** (20x20м, 100 клітинок).
   - Призначення: велика відкрита зона зберігання ресурсів, матеріалів та знарядь праці (32 слоти).
...
3. **Дерев'яна хатина колоністів (`wooden_hut.tres`):**
   - Розмір: **12x12 тайлів** (24x24м, 144 клітинки).
   - Призначення: житловий та адміністративний комплекс колонії, пункт організації будівельних робіт (`builder`).
